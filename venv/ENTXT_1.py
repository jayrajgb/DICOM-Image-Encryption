...
# Master
def generate_master_key_from_password(password, salt):
    kdf = PBKDF2HMAC(
...
def encrypt_file_with_master_key(master_file,master_key):
    ...
    print("Key storage secured successfully.")


def decrypt_file_with_master_key(master_file,master_key):
    with open(master_file, 'rb') as f:
    ...
    try:
        ...
        print("Invalid password or corrupted data. Decryption failed.")
...
def encrypt_dicom_files_in_directory(directory_path, key_file_path):
    ...
    print("All DICOM files in the directory encrypted and keys are stored.")
...
while True:
    ...
    if choice == "1":
        ...
        password = input("Enter a password to secure the stored keys: ")
        salt = os.urandom(16)
        master_key = generate_master_key_from_password(password, salt)
        encrypt_file_with_master_key(master_key)
        print("Key storage file encrypted successfully.")

    elif choice == "4":
        key_file_path = input("Enter the path of the key file containing decryption keys: ")
        password = input("Enter a password to decrypt the storage file: ")
        salt = os.urandom(16)
        master_key = generate_master_key_from_password(password, salt)
        decrypt_file_with_master_key(master_key)
